...
function AccessControl({sbUrl,tok,currentUser}){
  ...
  const[selectedRole,setSelectedRole]=useState("upperman");
  const[form,setForm]=useState({name:"",email:"",password:"",role:"upperman",status:"Active",notes:""});
  const roles=["admin","upperman","bottom_worker","finishman","staff","viewer"];
  const roleLabel=r=>({admin:"Admin",upperman:"Upperman",bottom_worker:"Bottom Man",finishman:"Finishman",staff:"Staff",viewer:"Viewer"}[r]||r);
  const headers={"apikey":SB_KEY,"Authorization":`Bearer ${tok}`,"Content-Type":"application/json"};
  const readJson=async r=>{const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.message||j?.details||`Request failed: ${r.status}`);return j;};
  const load=useCallback(async()=>{
    setLoading(true);setError("");
    try{
      const[m,p]=await Promise.all([
        fetch(`${sbUrl}/rest/v1/team_members?select=*&order=id.asc`,{headers}).then(readJson),
        fetch(`${sbUrl}/rest/v1/role_permissions?select=*&order=role.asc,module_id.asc`,{headers}).then(readJson)
      ]);
      setMembers(Array.isArray(m)?m:[]);setPermissions(Array.isArray(p)?p:[]);
    }catch(e){setError(e.message||"Could not load Team & Access.");}
    finally{setLoading(false);}
  },[sbUrl,tok]);
  useEffect(()=>{load();},[load]);

  const createLogin=async()=>{
    if(!form.email.trim()||!form.password){setError("Email and temporary password are required.");return;}
    setSaving(true);setError("");setMessage("");
    try{
      await fetch(`${sbUrl}/rest/v1/rpc/admin_upsert_team_login`,{
        method:"POST",headers,body:JSON.stringify({
          p_email:form.email.trim().toLowerCase(),p_password:form.password,
          p_name:form.name.trim()||null,p_role:form.role,p_status:form.status,p_notes:form.notes.trim()||null
        })
      }).then(readJson);
      setMessage(`Login saved for ${form.email.trim().toLowerCase()}.`);
      setForm({name:"",email:"",password:"",role:"upperman",status:"Active",notes:""});
      await load();
    }catch(e){setError(e.message||"Could not create login.");}
    finally{setSaving(false);}
  };
  const patchMember=async(id,changes)=>{
    setSaving(true);setError("");setMessage("");
    try{
      await fetch(`${sbUrl}/rest/v1/team_members?id=eq.${encodeURIComponent(id)}`,{
        method:"PATCH",headers:{...headers,Prefer:"return=representation"},body:JSON.stringify({...changes,updated_at:new Date().toISOString()})
      }).then(readJson);
      setMembers(rows=>rows.map(m=>String(m.id)===String(id)?{...m,...changes}:m));
      setMessage("Team member updated.");
    }catch(e){setError(e.message||"Could not update member.");}
    finally{setSaving(false);}
  };
  const permissionFor=(role,moduleId)=>permissions.find(p=>p.role===role&&p.module_id===moduleId)?.access_level||"none";
  const savePermission=async(moduleId,accessLevel)=>{
    setSaving(true);setError("");setMessage("");
    try{
      const payload={role:selectedRole,module_id:moduleId,access_level:accessLevel,updated_at:new Date().toISOString()};
      const rows=await fetch(`${sbUrl}/rest/v1/role_permissions?on_conflict=role,module_id`,{
        method:"POST",headers:{...headers,Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify(payload)
      }).then(readJson);
      const saved=Array.isArray(rows)?rows[0]:rows;
      setPermissions(all=>[...all.filter(p=>!(p.role===selectedRole&&p.module_id===moduleId)),saved||payload]);
      setMessage(`${roleLabel(selectedRole)} access updated.`);
    }catch(e){setError(e.message||"Could not update permission.");}
    finally{setSaving(false);}
  };
  const modules=[{id:"dashboard",l:"Dashboard"},...ALL_NAV.filter(i=>i.id!=="__access").map(i=>({id:i.id,l:i.l}))];
  const input={...C.inp,padding:"9px 10px",fontSize:12};
  return <div style={{height:"100%",overflowY:"auto",padding:24,background:C.bg}}>
    <div style={{maxWidth:1180,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:16,marginBottom:20}}>
        <div><h1 style={{margin:0,fontSize:24,color:C.text}}>Team & Access</h1><p style={{margin:"5px 0 0",fontSize:13,color:C.sub}}>Create workshop logins, activate or pause members, and control access by role.</p></div>
        <div style={{fontSize:11,color:C.dim,textAlign:"right"}}>Signed in as<br/><b style={{color:C.sub}}>{currentUser?.email||"Admin"}</b></div>
      </div>
      {error&&<div style={{padding:11,marginBottom:14,border:"1px solid #FCA5A5",background:"#FEF2F2",color:"#991B1B",fontSize:12}}>{error}</div>}
      {message&&<div style={{padding:11,marginBottom:14,border:"1px solid #86EFAC",background:"#F0FDF4",color:"#166534",fontSize:12}}>{message}</div>}
      {loading?<div style={{padding:40,textAlign:"center",color:C.dim}}>Loading team access…</div>:<>
        <div style={{background:C.card,border:`1px solid ${C.border}`,padding:18,marginBottom:20}}>
          <h2 style={{margin:"0 0 14px",fontSize:15}}>Create or reset a team login</h2>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1.2fr 1fr 1fr 1fr",gap:10}}>
            <input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Full name" style={input}/>
            <input type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="Email address" style={input}/>
            <input type="password" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} placeholder="Temporary password" style={input}/>
            <select value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))} style={input}>{roles.map(r=><option key={r} value={r}>{roleLabel(r)}</option>)}</select>
            <button onClick={createLogin} disabled={saving} style={{background:C.accent,border:"none",padding:"9px 12px",fontWeight:800,cursor:saving?"wait":"pointer"}}>{saving?"Saving…":"Save Login"}</button>
          </div>
          <p style={{margin:"8px 0 0",fontSize:11,color:C.dim}}>Using an existing email resets that member’s password and updates their role.</p>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1.1fr .9fr",gap:20,alignItems:"start"}}>
          <div style={{background:C.card,border:`1px solid ${C.border}`}}>
            <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`,fontWeight:800,fontSize:14}}>Team Members ({members.length})</div>
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>{["Member","Role","Status","Login"].map(h=><th key={h} style={{textAlign:"left",fontSize:10,color:C.dim,padding:"9px 12px",borderBottom:`1px solid ${C.border}`}}>{h}</th>)}</tr></thead>
              <tbody>{members.map(m=>{const active=m.is_active!==false&&String(m.status||"").toLowerCase()!=="inactive";return <tr key={m.id} style={{borderBottom:`1px solid ${C.border}`}}>
                <td style={{padding:"10px 12px"}}><b style={{fontSize:12}}>{m.name||m.full_name||m.email}</b><div style={{fontSize:10,color:C.dim}}>{m.email}</div></td>
                <td style={{padding:"10px 12px"}}><select value={m.role||"viewer"} disabled={saving} onChange={e=>patchMember(m.id,{role:e.target.value})} style={{...input,minWidth:115}}>{roles.map(r=><option key={r} value={r}>{roleLabel(r)}</option>)}</select></td>
                <td style={{padding:"10px 12px"}}><button disabled={saving} onClick={()=>patchMember(m.id,{is_active:!active,status:active?"Inactive":"Active"})} style={{border:"none",padding:"6px 9px",fontSize:11,fontWeight:700,cursor:"pointer",background:active?"#F0FDF4":"#FEF2F2",color:active?"#15803D":"#B91C1C"}}>{active?"Active":"Inactive"}</button></td>
                <td style={{padding:"10px 12px",fontSize:11,color:m.auth_user_id?C.ok:C.dim}}>{m.auth_user_id?"Created":"Not created"}</td>
              </tr>;})}</tbody>
            </table></div>
          </div>
          <div style={{background:C.card,border:`1px solid ${C.border}`}}>
            <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`}}>
              <div style={{fontWeight:800,fontSize:14,marginBottom:8}}>Role Permissions</div>
              <select value={selectedRole} onChange={e=>setSelectedRole(e.target.value)} style={input}>{roles.map(r=><option key={r} value={r}>{roleLabel(r)}</option>)}</select>
            </div>
            <div style={{maxHeight:520,overflowY:"auto"}}>{modules.map(mod=><div key={mod.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,padding:"8px 12px",borderBottom:`1px solid ${C.border}`}}>
              <span style={{fontSize:12,color:C.text}}>{mod.l}</span>
              <select value={permissionFor(selectedRole,mod.id)} disabled={saving||selectedRole==="admin"} onChange={e=>savePermission(mod.id,e.target.value)} style={{...input,width:100}}>
                {["none","view","edit","full"].map(a=><option key={a} value={a}>{a[0].toUpperCase()+a.slice(1)}</option>)}
              </select>
            </div>)}</div>
            {selectedRole==="admin"&&<p style={{margin:12,fontSize:11,color:C.dim}}>Admin always has full access.</p>}
          </div>
        </div>
      </>}
    </div>
